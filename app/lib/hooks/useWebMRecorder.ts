...
export function useWebMRecorder(
  ...
  const timeSlice = config.timeSlice || 1000; // 1 second chunks
  const maxDuration = config.maxDuration || 30000; // 30 seconds max
  const minBlobSize = config.minBlobSize || 1024; // 1KB minimum
